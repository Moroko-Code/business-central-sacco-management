...
Report 51516298 "Data Sheet ADJ Deposits"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Data Sheet ADJ Deposits.rdlc';

    dataset
    {
        dataitem("Data Sheet Main"; "Data Sheet Main")
        {
            DataItemTableView = sorting("Sort Code") order(ascending) where(Source = filter(BOSA), "Transaction Type" = filter(<> "ADJUSTMENT LOAN"));
            ...
            begin
                ...
                if EmployerS.Get(Employer) then
                ...
                if LOANS.Find('-') then
                    repeat
                        if LOANS."Repayment Start Date" <> 0D then
                ...
                if "Data Sheet Main"."Transaction Type" = "Data Sheet Main"."transaction type"::"FRESH FEED" then
