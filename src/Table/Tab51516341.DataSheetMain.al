...
Table 51516341 "Data Sheet Main"
{
    fields
    {
        ...
        field(5; "Amount ON"; Decimal)
        {
            ...
            begin
                ...
                if StrLen("PF/Staff No") = 10 then
                    PTEN := CopyStr("PF/Staff No", 10)
                else if StrLen("PF/Staff No") = 9 then
                    PTEN := CopyStr("PF/Staff No", 9)
                else if StrLen("PF/Staff No") = 8 then
                    PTEN := CopyStr("PF/Staff No", 8)
                else if StrLen("PF/Staff No") = 7 then
                    PTEN := CopyStr("PF/Staff No", 7)
                else if StrLen("PF/Staff No") = 6 then
                    PTEN := CopyStr("PF/Staff No", 6)
                else if StrLen("PF/Staff No") = 5 then
                    PTEN := CopyStr("PF/Staff No", 5)
                else if StrLen("PF/Staff No") = 4 then
                    PTEN := CopyStr("PF/Staff No", 4)
                else if StrLen("PF/Staff No") = 3 then
                    PTEN := CopyStr("PF/Staff No", 3)
                else if StrLen("PF/Staff No") = 2 then
                    PTEN := CopyStr("PF/Staff No", 2)
                else if StrLen("PF/Staff No") = 1 then
        ...
        field(24; "Adjustment Type"; Option)
        {
            ...
            begin
                ...
                end else if "Adjustment Type" = "adjustment type"::"Additional Loan" then
                        "Type of Deduction" := 'ADDITIONAL LOAN'
                else if "Adjustment Type" = "adjustment type"::"BELA Loan" then
                    "Type of Deduction" := 'BELA LOAN'
                else if "Adjustment Type" = "adjustment type"::"Benevolent Fund" then
                    "Type of Deduction" := 'Benevolent Fund'
                else if "Adjustment Type" = "adjustment type"::"Defaulters Loan" then
                    "Type of Deduction" := 'Defaulters Loan'
                else if "Adjustment Type" = "adjustment type"::"Emergency Loan" then
                    "Type of Deduction" := 'Emergency Loan'
                else if "Adjustment Type" = "adjustment type"::"Entrance Fee" then
                    "Type of Deduction" := 'Entrance Fee'
                else if "Adjustment Type" = "adjustment type"::"Jitegemee Loan" then
                    "Type of Deduction" := 'Jitegemee Loan'
                else if "Adjustment Type" = "adjustment type"::"Normal Loan" then
                    "Type of Deduction" := 'Normal Loan'
                else if "Adjustment Type" = "adjustment type"::"School Fee Loan" then
                    "Type of Deduction" := 'School Fee Loan'
                else if "Adjustment Type" = "adjustment type"::Shares then
                    "Type of Deduction" := 'shares'
                else if "Adjustment Type" = "adjustment type"::"Lariba loan" then
                    "Type of Deduction" := 'Lariba Loan'
                else if "Adjustment Type" = "adjustment type"::"Chipukizi Loan" then
        ...
        field(69025; Installments; Code[30])
        {
        }
    }
    ...
    keys
    {
        key(Key1; "PF/Staff No", "Type of Deduction", "Remark/LoanNO", Date, "ID NO.")
        {
            Clustered = true;
