...
Report 51516244 "Loan Appraisal"
{
    DefaultLayout = RDLC;
    ...
    dataset
    {
        dataitem("Loans Register"; "Loans Register")
        {
            ...
            column(Cshares_3; Cshares * "Loans Register".Mulitiplier)
            {
            }
            column(Cshares_3__LOANBALANCE_BRIDGEBAL_LOANBALANCEFOSASEC; (Cshares * Mulitiplier) - LOANBALANCE + BRIDGEBAL - LOANBALANCEFOSASEC)
            {
            }
            ...
            column(AmountAfterOffset; AmountAfterOffset)
            {
            }
            ...
            column(Loan_Insurance; "Loan Insurance") { }
            column(TopUpFee; Round(TopUpFee, 1, '>')) { }
            ...
            dataitem("Loans Guarantee Details"; "Loans Guarantee Details")
            {
                ...
                begin
                    ...
                    if CustRecord.Get("Loans Guarantee Details"."Member No") then begin
                        ...
                        AvailableG := CustRecord."Current Shares";
                    end;
                    LoanG.Reset;
            ...
            dataitem("Loan Offset Details"; "Loan Offset Details")
            {
                ...
                begin
                    ...
                    if "Loans Register".Get("Loan Offset Details"."Loan No.") then
            ...
            begin
                ...
                if Cust.Get("Loans Register"."Client Code") then begin
                    ...
                    SalDetails.SetFilter(SalDetails.Code, 'KCA');
                    if SalDetails.Find('-') then
                ...
                if (Depx < "Loans Register"."Requested Amount") or ("Loans Register"."Requested Amount" <= CollateralGuarantee) then
                    Recomm := CollateralGuarantee;
                If (Depx >= "Loans Register"."Requested Amount") or ("Loans Register"."Requested Amount" <= CollateralGuarantee) then
                    Recomm := Msalary;
                ...
                IF BRIGEDAMOUNT > 0 THEN
                    IF ObjProductType.GET("Loans Register"."Loan Product Type") THEN
                        TopUpFee := (ObjProductType."Top Up Commision" * BRIGEDAMOUNT) / 100;

                Booster := 0;
                if LoanAp.Get("Loans Register"."Loan  No.") then
                ...
                if LoanType.Get("Loan Product Type") then begin
                    ...
                    if LoanApp."Top Up Amount" > 0 then
                    ...
                    //LOAN Charges
                    AmountAfterOffset := 0;
                    AmountAfterOffset := LOANBALANCE - BRIGEDAMOUNT;

                    LoanProcessingFee := SFactory.FnGetChargeFee("Loans Register"."Loan Product Type", "Loans Register"."Approved Amount", 'PROCESSING');
                    LoanInsurance := SFactory.FnGetChargeFee("Loans Register"."Loan Product Type", "Loans Register"."Approved Amount", 'LOGUARD');
                    Upfronts := LoanProcessingFee + LoanInsurance + LegalFee + DisbursementFee + "Deboost Commision" + ValuationFee + TopUpFee + TopUpComm + BRIGEDAMOUNT;
                    Netdisbursed := ("Approved Amount" - Round(Upfronts, 1));
                    ...
                    if "Approved Amount" > 0 then begin
                        Upfronts := LoanProcessingFee + LoanInsurance + "Deboost Commision" + LegalFee + DisbursementFee + ValuationFee + TopUpFee + TopUpComm + BRIGEDAMOUNT;
                        Netdisbursed := ("Approved Amount" - Round(Upfronts, 1));
                        //"Loan Disbursed Amount" := Netdisbursed;
                        "Loan Insurance" := Round(LoanInsurance, 1);
...
#pragma warning restore AL0275
    ...
    begin
        ...
        if ObjLoanCharge.FindSet then
            if (ObjLoanCharge."Use Perc" = true) then
