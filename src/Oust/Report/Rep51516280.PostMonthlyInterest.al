        ...
        dataitem("Loans Register"; "Loans Register")
        {
            ...
            begin
                ...
                if LoanType.Get("Loans Register"."Loan Product Type") then
                    if not LoanType."Charge Interest Upfront" then begin
                        PDate := "Loans Register".GetRangemax("Date filter");
                        SDATE := '..' + Format(PDate);
                        DocNo := Format(PostDate);
                        loanapp.Reset;
                        loanapp.SetRange("Loan  No.", "Loans Register"."Loan  No.");
                        loanapp.SetFilter("Date filter", SDATE);
                        if loanapp.Find('-') then
                            repeat
                                loanapp.CalcFields(loanapp."Outstanding Balance");
                                if loanapp."Outstanding Balance" > 0 then begin
                                    Intarate := LoanType."Interest rate" / 12;
                                    Intarate := Round(Intarate, Precision, Direction);
                                    Intarate := Intarate / 100;
                                    InterestAmount := loanapp."Outstanding Balance" * Intarate;

                                    Cust.Reset;
                                    Cust.SetRange("No.", "Loans Register"."Client Code");
                                    Cust.SetFilter(status, '%1', Cust.status::Active);
                                    Cust.SetFilter("Don't Charge Interest", '%1', false);
