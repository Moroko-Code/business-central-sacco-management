    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("Check-off Type"; "Check-off Type")
                {
                    ApplicationArea = all;
                }
        ...
        area(processing)
        {
            ...
            action("Post check off")
            {
                ...
                begin
                    ...
                    if "Posting date" = 0D then
                        Error('You must specify the Posting date.');
                    if "Check-off Type" = "Check-off Type"::Loans then
                    ...
                    if RcptBufLines.Find('-') then
                        repeat
                            ...
                            if "Check-off Type" = "Check-off Type"::Deposits then
                            ...
                            else if "Check-off Type" = "Check-off Type"::"Fixed Deposits" then
                                RunBal := FnRunFixedContribution(RcptBufLines, RunBal)

                            else if "Check-off Type" = "Check-off Type"::Loans then
                    ...
                    if RunningBalance > 0 then begin
                        ...
                        if (LoanApp."Outstanding Balance" > 0) then
                            if (LoanApp."Issued Date" <= LoanCutoffDate) then begin
                                ...
                                if AmountToDeduct >= LoanApp.Repayment then begin
                                    MonthlyRepay := LoanApp.Repayment;
                                    NewOutstandingBal := LoanApp."Outstanding Balance" - MonthlyRepay;
                                end else if AmountToDeduct < LoanApp.Repayment then begin
                                ...
                                if RunningBalance > ScheduleRepayment then
