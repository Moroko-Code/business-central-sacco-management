...
Page 51516268 "Bosa Paybill"
{
    DeleteAllowed = false;
    PageType = Card;
    SourceTable = "Paybill Processing Header";
    Caption = 'Employee Check-off Processing';
    ...
    layout
    {
        area(content)
        {
            ...
            part("Bosa receipt lines"; "Bosa Paybill Lines")
            {
                ...
                begin
                    ...
                    if RcptBufLines.Find('-') then
                        repeat
                            ...
                            RunBal := FnRunFixedContribution(RcptBufLines, RunBal);
                    ...
                    if RunningBalance > 0 then begin
                        ...
                        if (LoanApp."Outstanding Balance" > 0) then
                            if (LoanApp."Issued Date" <= LoanCutoffDate) then begin
                                ...
                                if AmountToDeduct >= LoanApp."Recommended Repayment" then begin
                                    MonthlyRepay := LoanApp."Recommended Repayment";
                                    NewOutstandingBal := LoanApp."Outstanding Balance" - MonthlyRepay;
                                end else
                                    if (AmountToDeduct >= LoanApp.Repayment) and (LoanApp."Recommended Repayment" = 0) then begin
                                        MonthlyRepay := LoanApp.Repayment;
                                        NewOutstandingBal := LoanApp."Outstanding Balance" - MonthlyRepay;
                                    end else if
                                     AmountToDeduct < LoanApp.Repayment then begin
                                ...
                                //GET SCHEDULE REPYAMENT--To uncomment later
                                /* Lschedule.Reset;
                                 Lschedule.SetRange("Loan No.", LoanApp."Loan  No.");
                                 if Lschedule.FindFirst then begin
                                     LoanApp.CalcFields(LoanApp."Outstanding Balance");
                                     ScheduleRepayment := Lschedule."Monthly Repayment";
                                     if ScheduleRepayment > LoanApp."Outstanding Balance" then
                                ...
                                if RunningBalance > MonthlyRepay then
                                    Gnljnline.Amount := MonthlyRepay * -1
                                else
    ...
    begin
        if RunningBalance > 0 then begin
            ...
            ObjMember.SetFilter(ObjMember."Registration Date", '>%1', 20241231D); //To Ensure deduction is for New Members Only
    ...
    begin
        if RunningBalance > 0 then begin
            ObjMember.Reset;
            ObjMember.SetRange("No.", ObjRcptBuffer."Member No");
            ObjMember.SetRange("Customer Type", ObjMember."customer type"::Member);
            if ObjMember.Find('-') then begin
                AmountToDeduct := 0;
    ...
    local procedure FnRunFixedContribution(ObjRcptBuffer: Record "ReceiptsProcessing_L-Checkoff"; RunningBalance: Decimal): Decimal
    var
        varTotalRepay: Decimal;
        varMultipleLoan: Decimal;
        varLRepayment: Decimal;
    ...
    begin
        if RunningBalance > 0 then begin
            ...
            if ObjMember.Find('-') then begin
                AmountToDeduct := 0;
                //AmountToDeduct := (ObjMember."Monthly Fixed Deposits Cont.");
                //if RunningBalance <= AmountToDeduct then
                AmountToDeduct := RunningBalance;
                LineN := LineN + 10000;
                Gnljnline.Init;
                ...
                Gnljnline."Document No." := "Document No";
                Gnljnline."Posting Date" := "Posting date";
                Gnljnline.Description := 'Fixed Assets Savings ' + ObjRcptBuffer."Member No";
                Gnljnline.Amount := AmountToDeduct * -1;
                Gnljnline.Validate(Gnljnline.Amount);
                Gnljnline."Transaction Type" := Gnljnline."transaction type"::"Fixed Deposits Savings";
                Gnljnline."Shortcut Dimension 1 Code" := 'BOSA';
                Gnljnline."Shortcut Dimension 2 Code" := ObjMember."Global Dimension 2 Code";
                Gnljnline.Validate(Gnljnline."Shortcut Dimension 1 Code");
                Gnljnline.Validate(Gnljnline."Shortcut Dimension 2 Code");
    ...
    //         if ObjMember.Find('-') then begin
    //             AmountToDeduct := 0;
    //             AmountToDeduct := (ObjMember."Housing Contribution");
    //             if RunningBalance <= AmountToDeduct then
    //                 AmountToDeduct := RunningBalance;
    ...
    //             Gnljnline.Amount := AmountToDeduct * -1;
    //             Gnljnline.Validate(Gnljnline.Amount);
    //             Gnljnline."Transaction Type" := Gnljnline."transaction type"::junior_1;
    //             Gnljnline."Shortcut Dimension 1 Code" := 'BOSA';
    //             Gnljnline."Shortcut Dimension 2 Code" := ObjMember."Global Dimension 2 Code";
    //             Gnljnline.Validate(Gnljnline."Shortcut Dimension 1 Code");
    //             Gnljnline.Validate(Gnljnline."Shortcut Dimension 2 Code");
    ...
    //             Gnljnline.Amount := AmountToDeduct * -1;
    //             Gnljnline.Validate(Gnljnline.Amount);
    //             Gnljnline."Transaction Type" := Gnljnline."transaction type"::Junior_2;
    //             Gnljnline."Shortcut Dimension 1 Code" := 'BOSA';
    //             Gnljnline."Shortcut Dimension 2 Code" := ObjMember."Global Dimension 2 Code";
    //             Gnljnline.Validate(Gnljnline."Shortcut Dimension 1 Code");
    //             Gnljnline.Validate(Gnljnline."Shortcut Dimension 2 Code");
